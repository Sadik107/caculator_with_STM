...
int buffer_pos = 0; // Position in display buffer
/* USER CODE END PV */
...
void calculate(char key) {
    if (key >= '0' && key <= '9') {
        if (state == 0) { // Build and display num1
            num1 = num1 * 10 + (key - '0');
            display_buffer[buffer_pos++] = key;
            display_buffer[buffer_pos] = '\0';
            lcd_put_cur(0, 0);
            lcd_send_string(display_buffer);
        } else if (state == 1 || state == 2) { // Display num2 digits directly
            state = 2; // Ensure state is 2 after '+'
            display_buffer[buffer_pos++] = key;
            display_buffer[buffer_pos] = '\0';
            lcd_put_cur(0, 0);
            lcd_send_string(display_buffer);
        ...
        if (state == 0) {
            state = 1; // Operator entered
            operator = '+';
            display_buffer[buffer_pos++] = '+';
            display_buffer[buffer_pos] = '\0';
            lcd_put_cur(0, 0);
            lcd_send_string(display_buffer);
        }
        ...
        if (state == 2 && operator == '+') {
            // Extract num2 from display_buffer after '+'
            int i;
            for (i = 0; display_buffer[i] != '+'; i++); // Find '+'
            i++; // Move past '+'
            num2 = atoi(&display_buffer[i]); // Convert string after '+' to integer
            result = num1 + num2;
            sprintf(display_buffer, "%d", result);
            lcd_put_cur(1, 7); // After "Result:"
            lcd_send_string(display_buffer);
            HAL_Delay(1000); // Show result
        }
    } else if (key == 'C') {
        lcd_clear();
        lcd_put_cur(1, 0);
